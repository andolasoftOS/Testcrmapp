  ...
  return (
    <section className="doc_categories_guide_area sec_pad">
      ...
      <div className="container">
        ...
        <div className="row">
          {data.categories.nodes.slice().reverse().map((category, index) => (
          // {data.categories.nodes.map((category, index) => (
            <div className="col-lg-4 col-sm-6" key={category.id}>
              <div className="categories_guide_item wow fadeInUp" data-wow-delay={`${index * 0.2}s`}>
                <Image src={categoryIcons[index].src} alt={categoryIcons[index].alt} width={50} height={50} />
                <div className="doc_tag_title">
                  <h4>{category.name}</h4>
                </div>
                <ul className="list-unstyled tag_list">
                  {categoryLists[index].map(item => (
                    <li key={item.id}>
                      <i className="icon_check_alt2"></i> {item.title}
                    </li>
                  ))}
                </ul>
                ...
                <Link href={`/agile/cloud-category/${category.slug}`} legacyBehavior>
                  <a className="doc_border_btn">Learn More<i className="arrow_right"></i></a>
                </Link>  
              </div>
            </div>
...
export default DocCategoriesGuide;